...
/* add by chenfeng 20111018 */
void wakelock_print_begin(void);
void wakelock_print_stop(int mdelay);
...
/* add by chenfeng 20111018 */
struct wakelockprint_info {
	struct mutex lock;
	struct timer_list wakelockprint_timer;
};
char WakelockprintOnOff[4] = "off";
static struct class *wakelockprintonoff_class;
struct wakelockprint_info *hi;
...
/* sunyonggang */
/* Caller must acquire the list_lock spinlock */
static void yulong_print_active_locks(int type)
{
	struct wake_lock *lock;
	bool print_expired = true;
	int count = 0;
...
/* add by chenfeng 20111015 */
static void wakelockprint_event_timer_func(unsigned long data)
{
	yulong_print_active_locks(WAKE_LOCK_SUSPEND);
	mod_timer(&hi->wakelockprint_timer, jiffies + 1*HZ);
}

void wakelockprint_init(void)
{
	hi = kzalloc(sizeof(struct wakelockprint_info), GFP_KERNEL);
	init_timer(&hi->wakelockprint_timer);
	hi->wakelockprint_timer.function = &wakelockprint_event_timer_func;
	hi->wakelockprint_timer.expires = jiffies + 1*HZ;
}
...
void wakelock_print_begin(void)
{
	char *value1 = "on";
	char *value2 = "ono";
	if (!strcmp(value1, WakelockprintOnOff)) {
		;
		/* set_back_light(true); */
	} else if (!strcmp(value2, WakelockprintOnOff)) {
		add_timer(&hi->wakelockprint_timer);
		/* set_back_light(true); */
		printk(KERN_DEBUG "%s\n", __func__);
	}
}
...
void wakelock_print_stop(int mdelay)
{
	...
	if (!strcmp(value1, WakelockprintOnOff)) {
		...
		delaytime = mdelay;
		del_timer(&hi->wakelockprint_timer);
		if (delaytime > 0)
			mdelay(delaytime);
	}
}

static ssize_t wakelock_onoff_show(struct class *cls,
		struct class_attribute *attr, char *buf)
{
	printk(KERN_DEBUG "%s,wakelock_onoff status=%s\n", __func__, buf);
	return sprintf(buf, "%s\n", WakelockprintOnOff);
}
static ssize_t wakelock_onoff_store(struct class *cls,
		struct class_attribute *attr, const char *buf, size_t count)
{
	int err = 0;
	printk(KERN_DEBUG "Input is:%s:%d\n", buf, count);
	if (count > 4)
		err = -1;
	else
		sscanf(buf, "%s", WakelockprintOnOff);

	return count;
}

static CLASS_ATTR(wakelock_onoff, 0664,
		wakelock_onoff_show, wakelock_onoff_store);
...
#endif
	/* add by chenfeng for test wakelock */
	if (NULL == wakelockprintonoff_class) {
		wakelockprintonoff_class =
			class_create(THIS_MODULE, "wakelockprintonoff");
		if (IS_ERR(wakelockprintonoff_class))
			printk(KERN_ERR "%s: failed to create "
				"wakelockprintonoff_class\n", __func__);
	}
	error = class_create_file(wakelockprintonoff_class,
		(const struct class_attribute *)&class_attr_wakelock_onoff.attr);
	if (error < 0)
		printk(KERN_ERR"failed to create class_attr_wakelock_onoff\n");
