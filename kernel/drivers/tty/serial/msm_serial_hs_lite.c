...
#ifdef CONFIG_YULONG_MODULE_MANAGEMENT
struct module_management msm_uart_console;
#endif
...
static int msm_hsl_console_setup(struct console *co, char *options)
{
	...
	const struct msm_serial_hslite_platform_data *pdata = NULL;
...
/* sunyonggang */
#ifdef CONFIG_YULONG_MODULE_MANAGEMENT
	if (is_uart_console(port)) {
		msm_uart_console.private_data = port;
		yulong_register_module_management(&msm_uart_console);
	}
#endif
...
static int msm_serial_hsl_suspend(struct device *dev)
{
	...
	if (port) {
		...
		if (is_console(port)) {
			...
			pdata = pdev->dev.platform_data;
			if (pdata && pdata->config_gpio) {
				pr_debug("%s: pdata->uart_tx_gpio = %d\n",
					__func__, pdata->uart_tx_gpio);
				gpio_free(pdata->uart_tx_gpio);
				gpio_free(pdata->uart_rx_gpio);
			}
	...
	if (port) {
		...
		if (is_console(port)) {
			...
			if (pdata && pdata->config_gpio) {
				pr_debug("%s: pdata->uart_tx_gpio = %d\n",
					__func__, pdata->uart_tx_gpio);
				gpio_request(pdata->uart_tx_gpio,
						"UART_TX_GPIO");
				gpio_request(pdata->uart_rx_gpio,
						"UART_RX_GPIO");
			}
			msm_hsl_init_clock(port);
		}
...
#ifdef CONFIG_YULONG_MODULE_MANAGEMENT
int console_get(struct module_management *handler)
{
	int enable;
	struct uart_port *port = handler->private_data;
	struct uart_state *state = msm_hsl_uart_driver.state + port->line;

	if (!state->pm_state)
		enable = STATUS_ON;
	else if (state->pm_state == 3)
		enable = STATUS_OFF;
	else
		enable = -1;
...
int console_control(struct module_management *handler, unsigned int cmd)
{
	struct uart_port *port = handler->private_data;
	struct uart_state *state = msm_hsl_uart_driver.state + port->line;
...
static int __init msm_serial_hsl_init(void)
{
	...
	/* sunyonggang */
#ifdef CONFIG_YULONG_MODULE_MANAGEMENT
	msm_uart_console.name = "console";
	msm_uart_console.get  = console_get;
	msm_uart_console.control = console_control;
#endif
