...
struct lsm303d_status {
	...
	atomic_t mAccEncount;
	atomic_t mMagEncount;
	int temp_value_dec;
...
/*yanglin add for lsm303d power begin 2013-3-20*/
static int set_power(struct regulator *s, unsigned int voltage, bool enable)
{
    ...
    if(enable)
    {
        if(0 != voltage)
        {
            ...
            if (err)
             {
                printk(KERN_ERR"lsm %s: set_voltage failed \n", __func__);
                goto fail_s;
             }
        }
        ...
        if (err)
        {
            printk(KERN_ERR"lsm %s: enable error\n", __func__);
            goto fail_s;
        }
    }
    else
    {
        ...
        if (err)
         {
            printk(KERN_ERR"lsm%s: disable error\n", __func__);
            goto fail_s;
         }
    }
...
static int lsm303d_set_power(bool enable)
{
    ...
    {
        printk(KERN_INFO"lsm %s: value[%d]\n", __func__, enable);
...
err2:
    printk(KERN_ERR"lsm %s: err2 \n", __func__);
err1:
    printk(KERN_ERR"lsm %s: err1 \n", __func__);
    return -1;
}

static int lsm303d_i2c_request(void)
{
    gpio_request(I2C_DATA, "i2c_data");
    gpio_request(I2C_CLK, "i2c_clk");
    return 0;
}

static int lsm303d_i2c_free(void)
{
    gpio_free(I2C_DATA);
    gpio_free(I2C_CLK);
    return 0;
...
/*yanglin add for lsm303d power end 2013-3-20*/
...
static int lsm303d_acc_device_power_off(struct lsm303d_status *stat)
{
	int err;
	u8 buf[2];
	printk("lsm303d_acc_device_power_off \n");
	...
	if (stat->pdata_acc->power_off) {
		stat->pdata_acc->power_off(false);
	}
...
static int lsm303d_mag_device_power_off(struct lsm303d_status *stat)
{
	...
	if (stat->pdata_mag->power_off) {
		stat->pdata_mag->power_off(false);
	}
...
static int lsm303d_acc_device_power_on(struct lsm303d_status *stat)
{
	int err = -1;
	u8 buf[5];
	printk("lsm303d_acc_device_power_on  \n");
	if (stat->pdata_acc->power_on) {
		err = stat->pdata_acc->power_on(true);
		if (err < 0) {
			dev_err(&stat->client->dev,
...
static int lsm303d_mag_device_power_on(struct lsm303d_status *stat)
{
	...
	if (stat->pdata_mag->power_on) {
		err = stat->pdata_mag->power_on(true);
		if (err < 0) {
			dev_err(&stat->client->dev,
...
{
	atomic_dec(&stat->mAccEncount);
	if (atomic_read(&stat->mAccEncount) < 0)
		atomic_set(&stat->mAccEncount, 0);
	if (atomic_read(&stat->mAccEncount)== 0) {
...
{
	atomic_dec(&stat->mMagEncount);
	if (atomic_read(&stat->mMagEncount) < 0)
		atomic_set(&stat->mMagEncount, 0);
	if (atomic_read(&stat->mMagEncount) == 0 ) {
...
static int lsm303d_mag_get_data(struct lsm303d_status *stat, int *xyz)
{
	...
	work_mode[0] = 0x26;
	err = lsm303d_i2c_read(stat,work_mode,1);
	if (err < 0)
		return err;
	...
	printk("%s read y=%X %X(regH regL), x=%d(dec) [ug]\n",
		LSM303D_MAG_DEV_NAME, mag_data[3], mag_data[2], hw_d[1]);
	printk("%s read z=%X %X(regH regL), x=%d(dec) [ug]\n",
		...
		const struct i2c_device_id *id)
{
	...
	/*yanglin add for power begin */
	if (lsm303d_set_power(true)) {
		dev_err(&client->dev, "lsm303d_set_power true fail");
		goto exit_check_functionality_failed;
	} 
	lsm303d_i2c_request();
	/*yanglin add for power end */
...
/*yanglin add for lsm303d power begin 2013-3-20*/
	...
	if (NULL == pm8038_l9){
		printk(KERN_ERR"%s: pm8038_l9: regulator_get failed !\n", __func__);
		return -1;
	}
	pm8038_lvs2 = regulator_get(NULL, "8038_lvs2");
	if (NULL == pm8038_lvs2){
		printk(KERN_ERR"%s: pm8038_lvs2: regulator_get failed !\n", __func__);
		return -1;
	}
/*yanglin add for lsm303d power end 2013-3-20*/
