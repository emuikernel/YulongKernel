...
#define BMA250_PM_EN            1
#define BMA250_PWROFF_EN        0
...
static int set_power(struct regulator *s, unsigned int voltage, bool enable)
{
    ...
    if (enable) {
        if (voltage > 0) {
            ...
            if (err) {
                printk(KERN_ERR"%s: set_voltage failed \n", __func__);
                return -1;
            }
        }
        ...
        if (err) {
            printk(KERN_ERR"%s: enable error\n", __func__);
            return -1;
        }
    }
    else
    {
        ...
        if (err) {
            printk(KERN_ERR"%s: disable error\n", __func__);
            return -1;
        }
    }
#endif
...
static int bma250_power_init(void)
{
    power_state = false;
#if BMA250_PM_EN
...
static void bma250_power_deinit(void)
{
    power_state = false;
#if BMA250_PM_EN
