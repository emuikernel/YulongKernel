...
static int bmm_set_power(bool enable)
{
    int err = 0;
...
static int bmm_power_init(void)
{
    power_state = false;
#if BMM_PM_EN
    pm8038_l9 = regulator_get(NULL, "8038_l9");
    if (NULL == pm8038_l9) {
        PERR("%s: regulator_get failed !\n", __func__);
        return -1;
    }
...
static void bmm_power_deinit(void)
{
    power_state = false;
#if BMM_PM_EN
...
static int bmm_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	PINFO("%s: function entrance \r\n", __func__);
	if (bmm_set_power(true)) {
		PERR("%s: bmm_set_power(true) fail \r\n", __func__);
		return -1;
	}
	msleep(10);
...
exit_err_clean:
	if (err) {
        ...
        #if BMM_PWROFF_EN
		if (bmm_set_power(false)) {
        		PERR("%s: bmm_set_power(false) fail \r\n", __func__);
    		}
        #endif
		bmm_client = NULL;
	}
