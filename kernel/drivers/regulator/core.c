...
static struct class *regulatorrintonoff_class;
...
/* chenfeng */
void regulatorstatusprint_yl(int force)
{
	struct regulator *regulator = NULL;
	struct regulator_dev *rdev;
	int voltage = 0;
	...
	list_for_each_entry(rdev, &regulator_list, list) {
		...
		pr_debug("name=%s,voltage=%duV, is_eabled=%d\n",
				rdev->desc->name, voltage, enabled);
		list_for_each_entry(regulator, &rdev->consumer_list, list) {
			pr_debug("supply_name=%s, dev_name=%s, eable_times=%d,[%d-%d]uV, %duA\n",
			regulator->supply_name,
			regulator->dev ? dev_name(regulator->dev) : "NULL",
			regulator->enabled, regulator->min_uV,
			regulator->max_uV, regulator->uA_load);
		}
...
static CLASS_ATTR(regulator_onoff, 0664,
		regulator_onoff_show, regulator_onoff_store);

static ssize_t regulator_enable_store(struct class *cls,
		struct class_attribute *attr, const char *buf, size_t count)
{
	int err = 0;
	...
	list_for_each_entry(rdev, &regulator_list, list) {
		pr_debug("name=%s\n", rdev->desc->name);
		list_for_each_entry(regulator, &rdev->consumer_list, list) {
			pr_debug("supply_name=%s\n", regulator->supply_name);
			if (strncmp(buf, regulator->supply_name, count-1) == 0) {
				if (rdev->desc->ops->is_enabled)
					enabled = rdev->desc->ops->is_enabled(rdev);
				pr_debug("find supply %s in %s state\n",
					buf, enabled ? "enable" : "disable");
				err = regulator_enable(regulator);
				if (err)
					pr_err("enable %s failed, err=%d\n",
						buf, err);
				else
					pr_debug("enable %s success, err=%d\n",
						buf, err);
				err = 1;
				break;
			}
...
static CLASS_ATTR(regulator_enable, 0664, NULL, regulator_enable_store);

static ssize_t regulator_disable_store(struct class *cls,
		struct class_attribute *attr, const char *buf, size_t count)
{
	int err = 0;
...
static CLASS_ATTR(regulator_disable, 0664, NULL, regulator_disable_store);
...
unlock:
	...
	/* add by chenfeng */
	if (NULL == regulatorrintonoff_class) {
		regulatorrintonoff_class =
			class_create(THIS_MODULE, "regulatorprintonoff");
		if (IS_ERR(regulatorrintonoff_class))
			pr_err("%s: failed to create regulatorrintonoff_class\n", __func__);
	}
	error = class_create_file(regulatorrintonoff_class,
		(const struct class_attribute *)&class_attr_regulator_onoff.attr);
	if (error < 0)
		pr_err("failed to create class_attr_regulator_onoff\n");

	error = class_create_file(regulatorrintonoff_class,
		(const struct class_attribute *)&class_attr_regulator_enable.attr);
	if (error < 0)
		pr_err("failed to create class_attr_regulator_enable\n");

	error = class_create_file(regulatorrintonoff_class,
		(const struct class_attribute *)&class_attr_regulator_disable.attr);
	if (error < 0)
		pr_err("failed to create class_attr_regulator_disable\n");
...
#define NUM_MAX_REGG 128
static struct class *suspend_regulator;
static char *regulator_names;

static int regulator_set_vdd(const char *name, int enable)
{
	struct regulator *temp_vdd;
	int rc;

	rc = 0;
	if (enable) {
		temp_vdd = regulator_get(NULL, name);
		if (IS_ERR(temp_vdd)) {
			pr_err("could not get temp_vdd, rc = %ld, line = %d\n",
				PTR_ERR(temp_vdd), __LINE__);
			return -ENODEV;
		}
		rc = 0;

		rc = regulator_enable(temp_vdd);
		if (rc) {
			pr_err("enable temp_vdd failed, rc=%d, line = %d\n",
				rc, __LINE__);
			return -ENODEV;
		}
	} else {
		...
		rc = 0;
		rc = regulator_force_disable(temp_vdd);
		if (rc) {
			pr_err("disable temp_vdd failed, rc=%d, line = %d\n",
				rc, __LINE__);
			return -ENODEV;
		}
	}
...
static int get_info(char *buf, char **name, char **en_di, int *count)
{
	int i;
	...
	*en_di = buf + i;
	...
	while ((buf[i] != ',') && (i < *count - 1))
		i++;

	buf[i] = '\0';
	*count = *count - i;

	if ((*name == NULL) && (*en_di == NULL))
		return -1;
	return 0;
}

void suspend_resume_regulator(int sus_res)
{
	char regu_name[NUM_MAX_REGG];
	char *pointer, *name, *en_di;
	int count ;
	int max_num = NUM_MAX_REGG;

	if (regulator_names == NULL)
		return;

	name = NULL;
	en_di = NULL;
	count = max_num;
	memset(regu_name, 0, max_num);
	strcpy(regu_name, regulator_names);

	for (;;) {
		pointer = regu_name + (max_num - count);
		if (get_info(pointer, &name, &en_di, &count))
			break;
		if (!strcmp(en_di, "enble")) {
			if (regulator_set_vdd(name, !sus_res))
				pr_err("Set regulator error func = %s.\n",
					__func__);
		} else if (!strcmp(en_di, "disable")) {
			if (regulator_set_vdd(name, sus_res))
				pr_err("Set regulator error func = %s.\n",
					__func__);
		} else {
			pr_err("Input error func = %s.\n", __func__);
			break;
		}
		pr_err("%s: %s = %s,count = %d\n",
			__func__, name, en_di, count);
...
static ssize_t set_show(struct class *cls, struct class_attribute *attr,
		char *buf)
{
	return sprintf(buf, "%s\n", "example as follow:\necho 8038_l9=disable,8038_lvs1=disable,8038_l11=disable > /sys/class/suspend_regulator/set\n");
}

static ssize_t set_store(struct class *cls, struct class_attribute *attr,
		const char *buf, size_t count)
{
	if (regulator_names == NULL)
		regulator_names = kmalloc(NUM_MAX_REGG, GFP_KERNEL);
	memset(regulator_names, 0, NUM_MAX_REGG);
	sscanf(buf, "%s", regulator_names);
	pr_debug("Input is %s\n", regulator_names);
	return count;
}

static CLASS_ATTR(set, 0664, set_show, set_store);

static int __init suspend_regulator_init(void)
{
	int error = 0;

	if (NULL == suspend_regulator) {
		suspend_regulator =
			class_create(THIS_MODULE, "suspend_regulator");

		if (IS_ERR(suspend_regulator)) {
			pr_err("%s: failed to create suspend_regulator\n",
				__func__);
		}
	}
	error = class_create_file(suspend_regulator,
		(const struct class_attribute *)&class_attr_set.attr);
	if (error < 0)
		pr_err("failed to create class_attr_suspend_regulator onoff\n");

	return 0;
}
subsys_initcall(suspend_regulator_init);
