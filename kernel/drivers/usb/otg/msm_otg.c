...
static void msm_otg_notify_charger(struct msm_otg *motg, unsigned mA)
{
	...
	if (g && g->is_a_peripheral) {
		pr_debug("usb:msm_otg_notify_charger return\n");
		return;
	}
	...
	if (motg->cur_power == mA) {
		pr_debug("USB:motg->cur_power == mA and return mA==%d\n", mA);
		return;
	}

	dev_info(motg->phy.dev, "USB:Avail curr from USB = %u\n", mA);
	...
	if (msm_otg_notify_power_supply(motg, mA)) {
		pr_debug("usb:pm8921_charger_vbus_draw Start\n");
		pm8921_charger_vbus_draw(mA);
		pr_debug("usb:pm8921_charger_vbus_draw End\n");
	}
...
static int msm_otg_set_power(struct usb_phy *phy, unsigned mA)
{
	...
	pr_debug("usb:msm_otg_set_power mA==%d\n", mA);
	return 0;
}
...
static void msm_otg_start_peripheral(struct usb_otg *otg, int on)
{
	...
	if (on) {
		...
		pr_debug("usb:start_peripheral gadget on\n");
		/*
		 * Some boards have a switch cotrolled by gpio
		...
		pr_debug("usb:start_peripheral gadget off\n");
...
static void msm_otg_init_sm(struct msm_otg *motg)
{
	...
	case USB_OTG:
		if (pdata->otg_control == OTG_USER_CONTROL) {
			pr_debug("USB:----->OTG_USER_CONTROL\n");
			if (pdata->default_mode == USB_HOST) {
				pr_debug("USB:----->default_mode == USB_HOST\n");
				clear_bit(ID, &motg->inputs);
			} else if (pdata->default_mode == USB_PERIPHERAL) {
				pr_debug("USB:----->default_mode == USB_PERIPHERAL\n");
				set_bit(ID, &motg->inputs);
		...
		break;
	case USB_HOST:
		pr_debug("usb:--->USB_HOST %s\n", __func__);
		clear_bit(ID, &motg->inputs);
		break;
	case USB_PERIPHERAL:
		pr_debug("usb:--->USB_PERIPHERAL %s\n", __func__);
		set_bit(ID, &motg->inputs);
		if (pdata->otg_control == OTG_PHY_CONTROL) {
			if (otgsc & OTGSC_BSV)
				set_bit(B_SESS_VLD, &motg->inputs);
...
static void msm_otg_sm_work(struct work_struct *w)
{
		...
		if (test_bit(MHL, &motg->inputs)) {
			...
			case USB_CHG_STATE_DETECTED:
				...
				case USB_SDP_CHARGER:
					if(!slimport_is_connected()) {
						/* juzhitao 20120920
						 * for NON_STANDARD_WALL_CHARGER
						 */
						msm_otg_notify_charger(motg, 600);
						msm_otg_start_peripheral(otg, 1);
						otg->phy->state =
							OTG_STATE_B_PERIPHERAL;
					}
			...
			pr_debug("usb:chg_work cancel complete");
			motg->chg_state = USB_CHG_STATE_UNDEFINED;
			motg->chg_type = USB_INVALID_CHARGER;
			msm_otg_notify_charger(motg, 0);
			msm_otg_reset(otg->phy);
	...
	case OTG_STATE_B_PERIPHERAL:
		if (!test_bit(ID, &motg->inputs) ||
				...
				test_bit(ID_B, &motg->inputs) ||
				!test_bit(B_SESS_VLD, &motg->inputs)) {
			pr_debug("usb:OTG_STATE_B_PERIPHERAL  111\n");
			...
			pr_debug("usb:OTG_STATE_B_PERIPHERAL  222\n");
			...
			pr_debug("usb:OTG_STATE_B_PERIPHERAL  333\n");
			if (motg->caps & ALLOW_LPM_ON_DEV_SUSPEND) {
				pm_runtime_put_noidle(otg->phy->dev);
				pm_runtime_suspend(otg->phy->dev);
			}
		} else if (test_bit(ID_C, &motg->inputs)) {
			pr_debug("usb:OTG_STATE_B_PERIPHERAL  444\n");
			msm_otg_notify_charger(motg, IDEV_ACA_CHG_MAX);
		}
		break;
...
static void msm_otg_set_vbus_state(int online)
{
	...
	if (otg->phy->state >= OTG_STATE_A_IDLE) {
		pr_debug("usb:state >= OTG_STATE_A_IDLE %s\n", __func__);
		return;
	}
