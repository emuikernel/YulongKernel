			...
			char *buf)
{
	...
	strncpy(params_buff, "PRODUCTLINE", 11);
	pr_debug("%s: params_buff=%s\n", __func__, params_buff);
	ret = yl_params_kernel_read(params_buff, 512, 1);
	if (ret < 0)
...
#if 0
static ssize_t SN_store(struct device *pdev, struct device_attribute *attr,
			const char *buff, size_t size)
{
	int ret = 0;
	...
	strncpy(&params_buff[16], buff, 15);
	params_buff[31] = '\0';

	ret = yl_params_kernel_write(params_buff, 512, 1);
	if (ret < 0)
		pr_err("SN:write PRODUCTLINE to yulong params error\n");
