...
static struct pm8xxx_mpp_config_data pm8xxx_led_mpp5_config = {
...
#ifdef CONFIG_MSM8930_MPP
	/* add by youjin:to turn the led */
	if (value > 0 && value <= led->cdev.max_brightness) {
		tmp_level = (int)value/32;
		pr_debug("%s: led->mpp:%d,led->curr:%dbrightness:%d,level:%d\n",
			__func__, led->mpp, led->curr, value, tmp_level);
		pm8xxx_led_mpp5_config.control = PM8XXX_MPP_AOUT_CTRL_ENABLE;
		pm8xxx_led_mpp5_config.level = tmp_level;
		led->curr = tmp_level;
	} else {
		pm8xxx_led_mpp5_config.control = PM8XXX_MPP_AOUT_CTRL_DISABLE;
	}
	ret = pm8xxx_mpp_config(PM8038_MPP_PM_TO_SYS(led->mpp+1),
	&pm8xxx_led_mpp5_config);
#else
