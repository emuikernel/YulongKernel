...
u8 pm_gpio_config_get(struct gpio_chip *gpio_chip,  unsigned offset)
{
	u8 bank;
	u8 value = 0;
	int j;

	pr_debug("GPIO %d, base = %d", offset, offset - gpio_chip->base);
	for (j = 0; j < PM_GPIO_BANKS; j++) {
		bank = j << PM_GPIO_BANK_SHIFT;
		pm8xxx_writeb(gpio_chip->dev->parent,
			SSBI_REG_ADDR_GPIO(offset - gpio_chip->base), bank);
		pm8xxx_readb(gpio_chip->dev->parent,
			SSBI_REG_ADDR_GPIO(offset - gpio_chip->base), &bank);
		pr_debug(" 0x%02x", bank);
		if (j == 3)
			value = bank;
	}
	pr_debug("\n");
