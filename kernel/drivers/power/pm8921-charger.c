...
struct pm8921_chg_chip {
	...
	/*added by shchj beginning for reporting batt info in suspend 20121107*/
	struct				wake_lock wlock;
	struct				wake_lock aclock;
	struct				work_struct monitor_work;
	ktime_t				last_poll;
	struct				alarm alarm;
	/*added by shchj 20130412*/
	bool				is_recharging;
	/*added by shchj ending for reporting batt info in suspend 20121107*/
};
...
static int g_wakeup_interval = 30*60;
...
static int is_battery_valid(struct pm8921_chg_chip *chip)
{
	...
	pr_debug("chg_debug id min=%lld max=%lld\n",
		chip->batt_id_min, chip->batt_id_max);
...
static void bms_notify(struct work_struct *work)
{
	...
	pr_debug("n->is_charging = %d n->is_battery_full = %d\n",
		n->is_charging, n->is_battery_full);
	if (n->is_charging) {
...
static void bms_notify_check(struct pm8921_chg_chip *chip)
{
	...
	pr_debug("fsm_state = %d new_is_charging = %d\n",
		fsm_state, new_is_charging);
...
static int get_prop_batt_status(struct pm8921_chg_chip *chip)
{
	...
	/*modified by shchj begining 20130412*/
	if (chip->is_recharging && is_usb_chg_plugged_in(chip))
		return POWER_SUPPLY_STATUS_FULL;
	/*modified by shchj ending 20130412*/
	...
	if (fsm_state == FSM_STATE_ON_CHG_HIGHI_1) {
		/*modified by shchj begining 20130418*/
		...
		/*modified by shchj ending 20130418*/
	}
	return batt_state;
}
...
static int get_prop_batt_capacity(struct pm8921_chg_chip *chip)
{
	...
	/*modified by shchj beginning 20130110*/
	if (percent_soc < 0)
		percent_soc = voltage_based_capacity(chip);
	/*modified by shchj ending 20130110*/
	...
	/*added by shchj begining 20130413*/
	/*soc is not decreasing in suspend when usb is inpluged*/
	if ((chip->is_recharging == true) &&
	    (percent_soc >= (chip->resume_charge_percent))) {
		percent_soc = 100;
	} else {
		chip->is_recharging = false;
	}
	/*added by shchj ending 20130413*/
...
/*added by shcj beginning 20130109 for geting OCV*/
static int get_prop_batt_ocv(struct pm8921_chg_chip *chip)
{
	...
	pr_err("get_prop_batt_ocv:ocv_uv =%d,ibatt_ua =%d.\n",
		ocv_uv, ibatt_ua);

	return ocv_uv;
}
/*added by shcj ending 20130109 for geting OCV*/
...
/*added by shchj beginning 20130117*/
#define SHUTDOWN_VOLTAGE_MV	3500
#define SHUTDOWN_DELTA_VOLTAGE_MV 10
/*added by shchj ending 20130117*/
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		/*modified by shchj beginning 20130109*/
		rc = get_prop_batt_ocv(chip);
		if (rc < 0) {
			rc = get_prop_battery_uvolts(chip);
			if (rc < SHUTDOWN_VOLTAGE_MV*1000)
				rc += SHUTDOWN_DELTA_VOLTAGE_MV*1000;
		}
		/*modified by shchj ending 20130109*/
		if (rc >= 0) {
			val->intval = rc;
			rc = 0;
		}
...
#if 0 /* safety_time is not initiazed */
	int rc;

	if (the_chip->safety_time != 0) {
		/* for wall charger */
		if (type == POWER_SUPPLY_TYPE_USB_DCP)
			the_chip->safety_time = 180;
		/* for usb charger */
		else if (type == POWER_SUPPLY_TYPE_USB)
			the_chip->safety_time = 240;

		pr_debug("usb_target_ma=%d, safety_time=%d, usb_psy type=%d\n",
				usb_target_ma, the_chip->safety_time,
...
/*added by shchj beginning 20121229
 *for using in BMS to calculate iavg_ma*/
int pm8921_get_usb_power_supply_type(void)
{
	if (!the_chip) {
		pr_err("called before init\n");
		return -EINVAL;
	}
	pr_debug("usb_target_ma = %d, usb_psy type = %d\n",
		usb_target_ma, the_chip->usb_psy.type);
	return the_chip->usb_psy.type;
}
EXPORT_SYMBOL_GPL(pm8921_get_usb_power_supply_type);
/*added by shchj beginning 20121229*/
...
static void handle_usb_insertion_removal(struct pm8921_chg_chip *chip)
{
	...
	pr_debug("usb_present = %d chip->usb_present = %d\n",
...
{
	pr_debug("fsm_state=%d,usb_target_ma=%d\n",
		pm_chg_get_fsm_state(data), usb_target_ma);

	if (usb_target_ma) {
		/*added by shchj 20130416*/
		wake_lock_timeout(&the_chip->aclock, 1 * HZ);
...
static void unplug_check_worker(struct work_struct *work)
{
	...
	if ((active_path & USB_ACTIVE_BIT) && usb_target_ma > 0 &&
		...
		if ((reg_loop & VIN_ACTIVE_BIT) &&
			...
			/* Commented out by shchj 20130128 */
...
static irqreturn_t chg_gone_irq_handler(int irq, void *data)
{
	...
	pr_debug("chg_gone=%d, usb_valid = %d,fsm_state=%d\n",
			...
			int vbat_batt_terminal_uv, int ichg_meas_ma)
{
	...
	if (!is_ext_charging(chip)) {
		...
		pr_debug("fsm_state = %d, vbat_batt_terminal_uv=%d\n",
			pm_chg_get_fsm_state(chip), vbat_batt_terminal_uv);
		if (fast_chg == 0)
...
#define SAFE_CURRENT_MA		1000 /* -cxj modify 121008 1500 */
...
static int __devinit pm8921_chg_hw_init(struct pm8921_chg_chip *chip)
{
	...
	pr_debug("chip->cold_thr=%d,chip->hot_thr=%d\n",
			chip->cold_thr, chip->hot_thr);
...
/*added by shchj beginning for reporting batt info in suspend 20121107*/
static void pm8921_battery_program_alarm(int seconds)
{
	...
	next = ktime_add(the_chip->last_poll, low_interval);

	alarm_start_range(&the_chip->alarm, next, ktime_add(next, slack));
}
static void pm8921_battery_work(struct work_struct *work)
{
	if (!the_chip) {
		pr_err("chip not yet initalized\n");
		return;
	...
	the_chip->last_poll = alarm_get_elapsed_realtime();

	pm8921_battery_program_alarm(g_wakeup_interval);

	wake_unlock(&the_chip->wlock);
}
void pm8921_battery_alarm(struct alarm *alarm)
{
	if (!the_chip) {
		pr_err("chip not yet initalized\n");
		return;
	...
	wake_lock(&the_chip->wlock);
	schedule_work(&the_chip->monitor_work);
}
EXPORT_SYMBOL(pm8921_battery_alarm);
/*added by shchj ending for reporting batt info in suspend 20121107*/
...
static int __devinit pm8921_charger_probe(struct platform_device *pdev)
{
	...
	/*added by shchj 20130416*/
	wake_lock_init(&chip->aclock, WAKE_LOCK_SUSPEND, "pm8921_ac");
	...
	/*added by shchj beginning 20130114*/
	rc = pm8921_usb_ovp_set_threshold(PM_USB_OV_6V);
	if (rc)
		pr_err("Failed to overvoltage threshold to 6volts rc=%d\n", rc);
	/*added by shchj ending 20130114*/
	...
	wake_lock_init(&chip->wlock, WAKE_LOCK_SUSPEND, "pm8921-battery");
	INIT_WORK(&chip->monitor_work, pm8921_battery_work);
	alarm_init(&chip->alarm, ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP,
			pm8921_battery_alarm);
