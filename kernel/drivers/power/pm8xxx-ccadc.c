...
struct pm8xxx_ccadc_chip {
	...
	int			eoc_irq;
	int			r_sense_uohm;
	/*added by shchj beginning 20130110*/
	struct wake_lock ccadc_lock;
	/*added by shchj ending 20130110*/
...
static void calibrate_ccadc_work(struct work_struct *work)
{
	...
	pr_debug("ccadc calbrate inter");
	/*modified by shchj beginning 20130110*/
	wake_lock(&chip->ccadc_lock);
	pm8xxx_calib_ccadc();
	wake_unlock(&chip->ccadc_lock);
	/*modified by shchj ending 20130110*/
	pr_debug("ccadc calbrate exit");
...
static int __devinit pm8xxx_ccadc_probe(struct platform_device *pdev)
{
	...
	the_chip = chip;
	/*added by shchj beginning 20130110*/
	wake_lock_init(&chip->ccadc_lock, WAKE_LOCK_SUSPEND, "pm8xxx-ccadc");
	/*added by shchj ending 20130110*/
