					...
					int usb_chg, unsigned int uv)
{
	...
	pr_debug("xoadc_v0625 = %d,xoadc_v125=%d\n",
		chip->xoadc_v0625, chip->xoadc_v125);
...
/*added by shchj beginning 20121201*/
static int adc_based_ocv(struct pm8921_bms_chip *chip, int *ocv);
#define PON_OCV_DELTA_MV 200
				...
				int batt_temp_decidegc)
{
	...
	if (chip->prev_last_good_ocv_raw == OCV_RAW_UNINITIALIZED) {
		...
		/*added by shchj beginning 20121201
		 *for adjust PON ocv when PON ocv is
		 *too unreasionable*/
		...
		pr_debug("PON_OCV_UV = %d,adc_ocv=%d\n",
			chip->last_ocv_uv, ocv_uv);
		if (abs(chip->last_ocv_uv-ocv_uv)/1000 > PON_OCV_DELTA_MV) {
			chip->last_ocv_uv = ocv_uv;
			raw->last_good_ocv_uv = chip->last_ocv_uv;
		}
		pr_debug("adjust PON_OCV_UV = %d\n", chip->last_ocv_uv);
		/*added by shchj beginning 20121201*/
		...
		chip->last_ocv_uv = raw->last_good_ocv_uv;
		chip->last_ocv_temp_decidegc = batt_temp_decidegc;
		pr_debug("pre_ocv!=last_ocv,last_cc = %d\n", chip->last_cc_uah);
	...
	if (chip->ocv_reading_at_100 != raw->last_good_ocv_raw) {
		pr_debug("ocv_reading_at_100=0x%x, last_good_ocv_raw=0x%x\n",
 ...
 *          (micro Amp hour)
 */
/*added by shchj 20130116*/
#define DELTA_R_SENSE_MOHM (5*1000)
static void calculate_cc_uah(struct pm8921_bms_chip *chip, int cc, int *val)
{
	...
	/*modified by shchj begining 20130116*/
	cc_uah = div_s64(cc_pvh,
		(chip->r_sense_uohm + DELTA_R_SENSE_MOHM));
	/*modified by shchj ending 20130116*/
	*val = cc_uah;
}
				...
				int *ret_pc_unusable)
{
	...
	pr_debug("For batt_temp=%d,i_ma = %d, unusable_rbatt = %d unusable_uv = %d unusable_pc = %d uuc = %d\n",
		batt_temp, i_ma, uuc_rbatt_uv, unusable_uv, pc_unusable, uuc);
				...
				int iavg_ua, int delta_time_s)
{
	...
	/* modified by shchj beginning 20121228
	 * for not accurate reporting 100 soc
	 * in wall charging
	 */
	pr_debug("iavg = %d\n", iavg_ma);
	if (iavg_ma < 0) {
		type = pm8921_get_usb_power_supply_type();
		if (type == POWER_SUPPLY_TYPE_USB_DCP)
			iavg_ma = -iavg_ma;
		else
			iavg_ma = MIN_IAVG_MA;
	}
	/*modified by shchj ending 20121228*/
				...
				int fcc_uah, int cc_uah, int uuc_uah)
{
	...
	/*modified by shchj 20121231*/
	chg_soc = linear_interpolate(chip->soc_at_cv, chip->ibat_at_cv_ua,
					100, -155000,
					ibat_ua);
	chg_soc = bound_soc(chg_soc);

	/*added by shchj beginning 20130118*/
	if (chg_soc > 100)
		chg_soc = 100;
	/*added by shchj ending 20130118*/
		...
		int rbatt, int fcc_uah, int uuc_uah, int cc_uah)
{
	...
	if (ibat_ua < 0 && pm8921_is_batfet_closed()) {
		...
		/*modified by shchj beginning 20130107
		 *for charging soc compensation,
		 *applying for usb and wall charger*/
		if ((chip->soc_at_cv == -EINVAL) &&
		    (vbat_uv <= chip->max_voltage_uv)) {
			delta_ocv_uv_limit = -delta_ocv_uv_limit;
			goto out;
		} else {
			goto out;
		}
		/*modified by shchj ending 20130107*/
	}
	...
	/*modified by shchj beginning 20130107
	 *for discharging soc compensation*/
	 if (ibat_ua > 0) {
		n = min(50, max(1 , soc + soc_est + last_soc_est));
		m = m + 10;
	 }
	/*modified by shchj beginning 20130107 */
	...
	pr_debug("slope of ocv=%d point:m*1000/(pc-pc_new)=%d/(%d-%d)=%d\n",
		chip->last_ocv_uv, m*1000, pc, pc_new, (m*1000)/(pc-pc_new));

	/*modified by shchj beginning 20130110
	 *for discharging soc compensation*/
	delta_soc = abs(soc-soc_est);
	if (ibat_ua > 0) {
		if (delta_soc == 1) {
			n = n/2;
		} else if (delta_soc >= 2 && delta_soc <= 3) {
			n = n/4;
			m = m+1;
		} else {
			n = n/4;
	...
	/*modified by shchj beginning 20130122*/
	if (n > 0) {
	...
	/*modified by shchj ending 20130122*/
...
static bool is_shutdown_soc_within_limits(struct pm8921_bms_chip *chip, int soc)
{
	...
	/*added by shchj begining 20130424*/
	if ((soc > chip->shutdown_soc) && chip->shutdown_soc >= 95) {
		shutdown_soc_invalid = 1;
		return 0;
	}

	if (usb_chg_plugged_in(chip) && chip->shutdown_soc < 90) {
		pr_debug("%s: pow on with usb plugged\n", __func__);
		chip->shutdown_soc_valid_limit = 10;
	}
	/*added by shchj ending 20130424*/
					...
					int batt_temp, int chargecycles)
{
	...
	static int firsttime = 1;
	/*anded by shchj beginning 20130110*/
	int pc_ocv;
	...
	/*anded by shchj beginning 20130110*/
	pc_ocv = calculate_pc(chip, raw->last_good_ocv_uv,
		batt_temp, chargecycles);
	/*anded by shchj ending 20130110*/
	...
	if (firsttime && soc != shutdown_soc
		...
		pr_debug("DONE for shutdown_soc = %d soc is %d, adjusted ocv from pon_ocv_uv=%d to last_ocv_uv= %duV\n",
			shutdown_soc, soc, chip->pon_ocv_uv, chip->last_ocv_uv);
	}
...
static int report_state_of_charge(struct pm8921_bms_chip *chip)
{
	...
	pr_err("calculated_soc = %d,last_soc = %d,start_percent = %d,charge_time_us = %d,catch_up_time_us = %d\n",
			  calculated_soc, last_soc, the_chip->start_percent,
			  the_chip->charge_time_us, the_chip->catch_up_time_us);
	...
	/*added by shchj beginning 20130112*/
	/* last_soc < soc  ... scale and catch up */
	if (last_soc != -EINVAL && the_chip->start_percent != -EINVAL &&
	   last_soc > soc)
		soc = last_soc;
	/*added by shchj ending 20130112*/

	/*In discharging mode, soc should be always decrease.*/
	if (last_soc != -EINVAL && the_chip->start_percent == -EINVAL &&
	    soc > last_soc) {
		pr_debug("discharging mode, soc > last_soc, return last_soc.\n");
		soc = last_soc;
	}
...
void pm8921_bms_invalidate_shutdown_soc(void)
{
	...
	/*added by shchj beginning 20121224
	 *for not undoing what the adjusting did,
	 *when batt is unpluged in power off*/
	if (the_chip && is_shutdown_soc_within_limits(the_chip, calculated_soc))
		return;
	/*added by shchj ending 20121224*/
...
palladium:
		pr_debug("palladium 1500 assumed\n");
		...
		/*modified by shchj beginning 20121219
		 *sf table could be null when no battery
		 *aging data is available*/
		chip->rbatt_sf_lut = palladium_1500_data.rbatt_sf_lut;
		/*modified by shchj ending 20121219*/
