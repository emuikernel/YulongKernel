...
struct pmic8xxx_pwrkey *pwr_key_wake;
static irqreturn_t pwrkey_press_irq(int irq, void *_pwrkey)
{
	...
	/* yangliang add to compute time of light-screen */
	pr_debug("start to light screen!!");

	return IRQ_HANDLED;
}

/* zhangchangchun 2012-10-9 */
void pwrkey_wake(void)
{
	input_report_key(pwr_key_wake->pwr, KEY_POWER, 1);
	input_sync(pwr_key_wake->pwr);
}
EXPORT_SYMBOL(pwrkey_wake);
/* end zhangchangchun */
...
static irqreturn_t pwrkey_release_irq(int irq, void *_pwrkey)
{
	...
	input_sync(pwrkey->pwr);
	/* yangliang add to compute time of put out screen */
	pr_debug("start to put out screen!!");
