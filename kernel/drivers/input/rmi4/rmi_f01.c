...
/********   randy add for power manager 20121204*********/
...
static int touch_pm_control(struct module_management *handler,unsigned int cmd);
static int touch_pm_get(struct module_management *handler);
static int touch_pm_workon_flag = SET_MODULE_ON;

static int touch_pm_control(struct module_management *handler,unsigned int cmd)
{
	struct rmi_function_container *fc = handler->private_data;

        if(cmd == SET_MODULE_ON)
        {
                printk(KERN_ERR "\n randy add for power manager 20121204 touch_pm_control  : Set Module On\n ");
                rmi_f01_resume(fc);

        }
        else if(cmd == SET_MODULE_OFF)
        {
                printk(KERN_ERR "\n randy add for power manager 20121204  touch_pm_control  : Set Module Off\n ");
                rmi_f01_suspend(fc);
...
static int touch_pm_get(struct module_management *handler)
{
        printk(KERN_ERR "\n  randy add for power manager 20121204  touch_pm_get : Get Module Stat \n");
        return touch_pm_workon_flag;
}
static struct module_management touch_pm = {
	   .name = "touch_pm",
	   .get = touch_pm_get,
	   .control = touch_pm_control,
};
...
static int rmi_f01_initialize(struct rmi_function_container *fc)
{
	...
	/********   randy add for power manager 20121204*********/
	#ifdef CONFIG_YULONG_MODULE_MANAGEMENT
	printk(KERN_ERR "\nrandy add for power manager 20121204 : yulong_register_module_management\n");
	touch_pm.private_data = fc;
	yulong_register_module_management(&touch_pm);
	#endif
	/********   randy add for power manager 20121204*********/
...
static int rmi_f01_suspend(struct rmi_function_container *fc)
{
	...
	/********   randy add for power manager 20121204*********/
#ifdef CONFIG_YULONG_MODULE_MANAGEMENT
	touch_pm_workon_flag = SET_MODULE_OFF;
#endif
/********   randy add for power manager 20121204*********/
...
static int rmi_f01_resume(struct rmi_function_container *fc)
{
	...
	/********   randy add for power manager 20121204*********/
#ifdef CONFIG_YULONG_MODULE_MANAGEMENT
	touch_pm_workon_flag = SET_MODULE_ON;
#endif
/********   randy add for power manager 20121204*********/
...
{
	int error;
	printk(KERN_ERR "== enter in %s by ck ==\n",__func__);
