...
//#define	TOUCH_DEBUG_MEG	1	//randy add for key
...
int key_code[KEY_NUM] ={
        KEY_MENU,
	KEY_HOME,
        KEY_BACK,
};

static bool key_status[KEY_NUM];
...
static void rmi_f11_rel_pos_report(struct f11_2d_sensor *sensor, u8 n_finger)
{
	...
	s8 x, y;
	s8 temp;
	#ifdef	TOUCH_DEBUG_MEG
printk(KERN_ERR "Rmi Enter In ===> %s \n",__func__);
	#endif
	...
	#ifdef	TOUCH_DEBUG_MEG
printk("Function 111:%s REL===>  x==%4d    y==%4d\n",__func__, x, y);
	#endif
	if (x || y) {
		printk("Function if (x || y) :%s REL===>  x==%4d    y==%4d\n",__func__, x, y);
...
#ifdef CONFIG_TW_DEBUG
/* yangliang add 20130201 */
					...
					u8 finger_state, u8 n_finger)
{
	...
	#ifdef	TOUCH_DEBUG_MEG
printk(KERN_ERR "Rmi Enter In ===> %s---finger_state= %d,  prev_state=%4d\n",__func__, finger_state, prev_state);
	#endif
	if (prev_state && !finger_state) {
		#ifdef	TOUCH_DEBUG_MEG
		printk("Release (Prev & Now) --> (%4d  &  %4d)\n",prev_state, finger_state);
		#endif
		/* this is a release */
		...
		/******************************randy  pan add for button ******************************/
	}
	else if (!prev_state && !finger_state)
	{
		#ifdef	TOUCH_DEBUG_MEG
		printk(KERN_ERR "Enter In ===> %s  Nothing report \n",__func__);
		#endif
		/* nothing to report */
		return 0;
	}
	else
	{
		...
		*/
		...
		/* yuhao add 0417 */
		if (w_max == 0)
			printk("TW IS UNORMAL0 x=%d; y=%d; w_max=%d\n",
				x, y, w_max);
		if (z == 0)
			printk("TW IS UNORMAL1 x=%d; y=%d; z=%d\n", x, y, z);
	}

//	pr_debug("%s: f_state[%d]:%d - x:%d y:%d z:%d w_max:%d w_min:%d\n",
//		__func__, n_finger, finger_state, x, y, z, w_max, w_min);
	//printk("Function :%s ABS===>  x==%4d    y==%4d\n",__func__, x, y);
	#ifdef	TOUCH_DEBUG_MEG
	printk("(%4d , %4d)\n", x, y);
	#endif
...
/* yangliang add 20130201 */
#ifdef CONFIG_TW_DEBUG
	if (press_flag == 0 && w_max != 0) {
		press_flag = 1;
		printk(KERN_ERR "TW IS PRESSED DOWN!!");
	} else if (press_flag == 1 && w_max == 0) {
		press_flag = 0;
		printk(KERN_ERR "TW IS PRESS UP!!");
	}
#endif
	/* yangliang add end20130201 */
...
static int rmi_f11_virtual_button_handler(struct f11_2d_sensor *sensor)
{
	...
	#ifdef	TOUCH_DEBUG_MEG
	printk("rmi enter in %s  ---> (%4d , %4d)\n",__func__, x, y);
	#endif

	if(button_off)
	{
		#ifdef	TOUCH_DEBUG_MEG
		printk("%s :Key= %4d---------Key OFF\n", __func__, code);
		#endif
		button_off = 0;
		return 0;
	}
	#ifdef	TOUCH_DEBUG_MEG
	printk("rmi enter in %s  Out of Key area---> (%4d , %4d)\n",__func__, x, y);
	#endif
	return 0;
}
#else
...
static int rmi_f11_virtual_button_handler(struct f11_2d_sensor *sensor)
{
	...
	if (sensor->sens_query.has_gestures &&
		...
		for (i = 0; i < virtualbutton_map->buttons; i++) {
			printk(KERN_ERR "Rmi Enter In Key %4d===> %s \n",i, __func__);
			if (INBOX(x, y, virtualbutton_map->map[i])) {
				...
				//randy add for virtual key
				printk("===enter in %s report Key = %4d(%4x)===\n",__func__, virtualbutton_map->map[i].code,virtualbutton_map->map[i].code);
				//randy add for virtual key
				return 0;
			}
		}
...
static void rmi_f11_finger_handler(struct f11_2d_sensor *sensor)
{
	...
	u8 i;
	/*yuhao add 0228*/
	static int prev_report_state = 0;
	int report_state = 0;		 
...
printk(KERN_ERR "Rmi Enter In ===> %s \n",__func__);
	...
	for (i = 0, finger_pressed_count = 0; i < sensor->nbr_fingers; i++) {
		...
		#ifdef	TOUCH_DEBUG_MEG
		printk(KERN_ERR "Rmi Enter In ===> %s---finger_state = %d\n",__func__, finger_state);
		#endif
		if (sensor->data.abs_pos)
			report_state |= rmi_f11_abs_pos_report(sensor, finger_state, i);
...
/*yuhao add 0228*/
	if(report_state & 0x01)
	{
		   input_sync(sensor->input);

	}
	else if(report_state & 0x10)
	{
		prev_report_state = report_state;
	}	
	else if(prev_report_state)
	{				   
		   input_mt_sync(sensor->input);						  
		   input_sync(sensor->input);
	}
...
static int f11_2d_construct_data(struct f11_2d_sensor *sensor)
{
	...
	if (query->has_rel) {
		//randy add for update
		printk("***************************%s: randy add Rel = %d\n",__func__, query->has_rel);
...
static int rmi_f11_initialize(struct rmi_function_container *fc)
{
	...
	*/
	...
	printk("%s: query_base_addr =0x%x ===by ck\n",__func__,query_base_addr);
	...
	for (i = 0; i < (f11->dev_query.nbr_of_sensors + 1); i++) {
		...
		if (rc < 0)
			return rc;
		printk(KERN_ERR "====MAX_X_POS is %x,%d ===\n",max_x_pos,max_x_pos);
	...
	for (i = 0; i < (f11->dev_query.nbr_of_sensors + 1); i++) {
		...
		* phys name for input device */
		...
		set_bit(EV_ABS, input_dev->evbit);
    		set_bit(INPUT_PROP_DIRECT, input_dev->propbit);//randy add for 2202 update key button 0929

		/**start:pc add.12.05.06**/
