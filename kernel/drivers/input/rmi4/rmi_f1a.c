...
int rmi_f1a_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
	struct rmi_device *rmi_dev = fc->rmi_dev;
	struct f1a_data *f1a = fc->data;
	//u8 data_base_addr = fc->fd.data_base_addr; //remove by ck
	int error;
	int button;

        /**start:pc modify.12.05.06**/
        //f1a->button_map[0] = KEY_BACK; //{KEY_BACK,KEY_HOME,KEY_MENU};
        //f1a->button_map[1] = KEY_HOME;
        //f1a->button_map[2] = KEY_MENU;
        /**end:pc modify.12.05.06**/

	/*add by ck for 5216 */
        f1a->button_map[0] = KEY_MENU;
        f1a->button_map[1] = KEY_HOME;
        f1a->button_map[2] = KEY_BACK;
	/*add by ck for 5216 */


	/* Read the button data. */
	//error = rmi_read_block(rmi_dev, data_base_addr, f1a->button_data_buffer,
	error = rmi_read_block(rmi_dev, 0x0200, f1a->button_data_buffer,
