...
int yulong_register_module_management(struct module_management *handler)
{
	...
	if (strlen(handler->name) >= (sizeof(struct modules_state) - 1)) {
		printk(KERN_ERR "%s:Error! device name %s is too long. "
			"It would load to an error!\n",
	...
	list_for_each_entry(pos, &module_management_handlers, link) {
		if (pos->name) {
			if (!strcmp(pos->name, handler->name)) {
				...
				printk(KERN_ERR "%s: %s is failed, "
					"there was a same name.\n",
					__func__, handler->name);
				return -1;
			}
		}
			...
			char *buf)
{
	...
	list_for_each_entry(pos, &module_management_handlers, link) {
		if (pos->name) {
			...
			((!state) ? "off" : ((state == 1) ? "on" : "unknow")));
			count += sprintf(buf + count, "%s:%s,", pos->name,
				((!state) ? "2" : ((state == 1) ? "1" : "0")));
		}
	}
	mutex_unlock(&module_management_lock);
...
static int handle_commad(struct module_management *pos, const char *cona)
{
	int state = -1;
	if (!strcmp(cona, "1")) {
		if (pos->control) {
...
static ssize_t power_modules_show(struct kobject *kobj,
...
static ssize_t power_modules_store(struct kobject *kobj,
