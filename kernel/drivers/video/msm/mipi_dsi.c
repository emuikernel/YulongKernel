...
#endif
	/*ykl add 20120621 enter in ulps mode*/
	status = MIPI_INP(MIPI_DSI_BASE + 0xa8);
	MIPI_OUTP(MIPI_DSI_BASE + 0xa8, status|0x13);
	msleep(10);
	/*end*/
...
static int mipi_dsi_on(struct platform_device *pdev)
{
	...
	if (!firstOn) {
		status = MIPI_INP(MIPI_DSI_BASE + 0xa8);
		MIPI_OUTP(MIPI_DSI_BASE + 0xa8, status|0x1300);
		msleep(10);
