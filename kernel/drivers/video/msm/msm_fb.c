...
unsigned long esd_timer_duration = (5 * HZ);   /* 5000 msecond */
extern uint8 monitor_reg_status(struct msm_fb_data_type *mfd, char mode);
static struct workqueue_struct *esd_work_queue;
...
static int msm_fb_probe(struct platform_device *pdev)
{
	...
	if (mfd->panel_info.type != DTV_PANEL &&
		...
		esd_work_queue = create_singlethread_workqueue("esd_wq");
		INIT_DELAYED_WORK(&mfd->esd_worker, esd_workqueue_handler);
	}
#endif
	/*end*/
...
/* ykl modefy for continus display */
static int unset_bl_level = 140;
			    ...
			    boolean op_enable)
{
	...
	case FB_BLANK_UNBLANK:
		if (!mfd->panel_power_on) {
			...
			if (ret == 0) {
				...
				if (mfd->panel_info.type != DTV_PANEL &&
				    mfd->panel_info.type != WRITEBACK_PANEL)
					queue_delayed_work(esd_work_queue,
						&mfd->esd_worker,
						esd_timer_duration);
#endif
			/*end*/
	...
	default:
		if (mfd->panel_power_on) {
			...
			if (mfd->panel_info.type != DTV_PANEL &&
			    mfd->panel_info.type != WRITEBACK_PANEL)
				__cancel_delayed_work(&mfd->esd_worker);
#endif
			/*end*/
...
/*ykl add*/
#if defined(CONFIG_YULONG_LCD_ESD) || defined(CONFIG_YULONG_MODULE_MANAGEMENT)
static int reset_display(struct msm_fb_data_type *mfd, int mode)
{
	struct msm_fb_panel_data *pdata =
		(struct msm_fb_panel_data *)mfd->pdev->dev.platform_data;
	struct early_suspend *h;

	list_for_each_entry(h, &(mfd->early_suspend.link), link) {
		if (h->level == 100)
			break;
	}

	if (!h) {
		pr_err("get fb suspend error\n");
		return -1;
	}
	if ((!pdata) || (!pdata->on) || (!pdata->off)) {
		printk(KERN_ERR "msm_fb_blank_sub: no panel operation detected!\n");
...
static int display_control(struct module_management *display, unsigned int cmd)
{
	struct msm_fb_data_type *mfd = container_of(display,
				struct msm_fb_data_type, module_management);
	int ret = 0;

	if (!mfd) {
		pr_err("get mfd error\n");
		return -ENOMEM;
	}
	if (cmd == SET_MODULE_ON)
		ret = reset_display(mfd, 1);
	else if (cmd == SET_MODULE_OFF)
		ret = reset_display(mfd, 0);

	return ret;
}

static int display_get(struct module_management *display)
{
	int val = 0;
	struct msm_fb_data_type *mfd = container_of(display,
				struct msm_fb_data_type, module_management);
...
/*ykl add*/
	...
	if (hdmi_prim_display ||
	    (mfd->panel_info.type != DTV_PANEL &&
	     mfd->panel_info.type != WRITEBACK_PANEL)) {
		mfd->module_management.name = "display";
		mfd->module_management.get = display_get;
		mfd->module_management.control = display_control;
		yulong_register_module_management(&mfd->module_management);
	}
#endif
...
/*ykl add for ESD*/
#ifdef CONFIG_YULONG_LCD_ESD
void esd_workqueue_handler(struct work_struct *work)
{
	struct delayed_work *delay_work = (struct delayed_work *)work;
	struct msm_fb_data_type *mfd =
		container_of(delay_work, struct msm_fb_data_type, esd_worker);
	pr_debug("%s: enter\n", __func__);
	if (monitor_reg_status(mfd, 0)) {
		pr_info("--oh,esd happen\n");
		reset_display(mfd, 0);
		reset_display(mfd, 1);
	}
	queue_delayed_work(esd_work_queue, &mfd->esd_worker,
			esd_timer_duration);
}
#endif
/*end*/
