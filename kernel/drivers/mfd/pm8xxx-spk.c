...
static int yulong_tpa_modem_on;
#endif
...
int pm8xxx_spk_enable(int enable)
{
	...
	if (val < 0)
		return val;
	/* modify by youjin to close the tpa of pm8xxx 2012-11-29 */
	if (enable) {
		val |= (1 << 3);
#ifdef CONFIG_YULONG_MODULE_MANAGEMENT
		yulong_tpa_modem_on = 1;
#endif
		pr_debug("enable pmic-spk val:%x\n", val);
	} else {
		val &= ~(1 << 3);
#ifdef CONFIG_YULONG_MODULE_MANAGEMENT
		yulong_tpa_modem_on = 0;
#endif
		pr_debug("disablevimdddd pmic-spk val:%x\n", val);
	}
...
#ifdef CONFIG_YULONG_MODULE_MANAGEMENT
static int tpa_spk_pm8xx_control(struct module_management *module_handler,
			unsigned int cmd)
{
	int ret = 0;
	if (cmd == SET_MODULE_ON) {
		ret = pm8xxx_spk_enable(1);
		pr_info("%s: TPA ON !\n", __func__);
	} else if (cmd == SET_MODULE_OFF)
		ret = pm8xxx_spk_enable(0);
	else
		pr_err("Unknow cmd of module_management\n");

	return ret;
}
static int tpa_spk_pm8xx_get(struct module_management *module_handler)
{
	int ret = 0;
	int val;
	if (yulong_tpa_modem_on) {
		ret = yulong_tpa_modem_on;
	} else {
		val = pm8xxx_spk_read(PM8XXX_SPK_CTL1_REG_OFF);
		if (0 == (val & (1<<3)))
			ret = 0;
...
static struct module_management tpa_spk_pm8xxx = {
	.name = "tpa_spk_pm8xx",
	.get = tpa_spk_pm8xx_get,
	.control = tpa_spk_pm8xx_control,
};
#endif
...
#ifdef CONFIG_YULONG_MODULE_MANAGEMENT
	yulong_register_module_management(&tpa_spk_pm8xxx);
#endif
...
err_handle:
	...
	kfree(the_spk_chip);
#ifdef CONFIG_YULONG_MODULE_MANAGEMENT
	yulong_unregister_module_management(&tpa_spk_pm8xxx);
#endif
	return ret;
}
