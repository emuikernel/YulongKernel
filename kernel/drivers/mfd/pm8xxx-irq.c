...
/* sunyonggang */
static int pm8xxx_irq_block(struct pm_irq_chip *chip, int block)
{
	int pmirq, irq, i, ret = 0;
	u8 bits;
	...
	for (i = 0; i < 8; i++) {
		if (bits & (1 << i)) {
			...
			if (desc && (desc->action) && desc->action->name)
				pr_debug("irq_num = %d name = %s\n",
						irq, desc->action->name);
			else
				pr_debug("irq_num = %d name = NULL\n", irq);
		}
	}
	return 0;
}

static int pm8xxx_irq(struct pm_irq_chip *chip, int master)
{
	u8 blockbits;
	int block_number, i, ret = 0;
	...
	for (i = 0; i < 8; i++)
		if (blockbits & (1 << i)) {
			...
			ret |= pm8xxx_irq_block(chip, block_number);
		}
	return ret;
}

int pm8xxx_resume_irq(void *data)
{
	struct pm_irq_chip *chip = data;
	u8 root;
	int i, ret, masters = 0;
