		...
		uint16_t gain, uint32_t line, int32_t luma_avg, uint16_t fgain)
{
	...
	//s_ctrl->func_tbl->sensor_group_hold_on(s_ctrl); remarked by wuchao for AEC error from QT
	...
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		...
		MSM_CAMERA_I2C_WORD_DATA);
	//s_ctrl->func_tbl->sensor_group_hold_off(s_ctrl); remarked by wuchao for AEC error from QT
	return 0;
}
		...
		uint16_t gain, uint32_t line, int32_t luma_avg, uint16_t fgain)
{
	...
	if (line > (fl_lines - offset))
		fl_lines = line + offset;
	CDBG("ov8825_write_pic_exp_gain: %d %d %d\n", fl_lines, gain, line);
...
static int32_t ov8825_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
	struct msm_camera_sensor_info *info = NULL;
...
static int32_t ov8825_sensor_setting(struct msm_sensor_ctrl_t *s_ctrl,
