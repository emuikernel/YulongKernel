...
#if 0
/* add by youjin: to open the debug */
#undef dev_dbg
#undef pr_debug
...
#ifdef CONFIG_SND_H2W_COOLPAD
/* add by youjin to  simulate a yulong  */
extern void simulate_hs_report(struct snd_soc_jack *jack, int status, int mask);
#endif
...
static const struct snd_kcontrol_new sitar_snd_controls[] = {
	...
	/* add by youjin: move form wcd9310.c to control micb cap;
	 * yulong get capless
	 */
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		/* added by maleijie 2012/11/13 for pop */
	...
	int vddio_switch)
{
	...
	case 1:
		if (sitar->mbhc_micbias_switched == 0 &&
			...
			pr_debug("%s: enable Mic Bias switch to VDDIO\n",
				__func__);
			sitar_codec_pause_hs_polling(codec);
			...
			pr_debug("%s: cfilt_k_val:[%d]\n",
				__func__, cfilt_k_val);
			snd_soc_update_bits(codec,
				sitar->mbhc_bias_regs.cfilt_val,
				0xFC, (cfilt_k_val << 2));
	...
	case 0:
		if (sitar->mbhc_micbias_switched) {
			pr_debug("%s: disable Mic Bias switch to VDDIO\n",
				__func__);
			if (sitar->mbhc_polling_active) {
				sitar_codec_pause_hs_polling(codec);
				mbhc_was_polling = true;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	pr_debug("%s %d\n", __func__, event);
	pr_debug("%s: %s reg:%x sitar->mbhc_bias_regs.ctl_reg%x\n",
		__func__, w->name, w->reg, sitar->mbhc_bias_regs.ctl_reg);
	switch (w->reg) {
	case SITAR_A_MICB_1_CTL:
		/*change by youjin:to deal with sencod mic*/
		if (strnstr(w->name, external2_text, 30)) {
			pr_debug("%s: second mic set\n", __func__);
	...
	case SITAR_A_MICB_2_CTL:
		pr_debug("%s: there is headset mic set\n", __func__);
		micb_int_reg = SITAR_A_MICB_2_INT_RBIAS;
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (w->reg == sitar->mbhc_bias_regs.ctl_reg) {
			...
			pr_debug("envent:PRE_PMU mbhc\n");
			SITAR_RELEASE_LOCK(sitar->codec_resource_lock);
		}

		snd_soc_update_bits(codec, w->reg, 0x86, 0x86);
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		/* add by youjin: to close the bias */
		snd_soc_update_bits(codec, w->reg, 0x86, 0x00);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		usleep_range(1000, 1000);  /* added by zhuhui for pop */
...
#ifdef CONFIG_SND_H2W_COOLPAD
	/*add by youjin to use yulong hw */
	simulate_hs_report(jack, status, mask);

	/* delete to avaid deadlock ,and just use yulong mode 2012-11-19 */
...
static int sitar_codec_enable_dac1(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	pr_debug("%s %s %d\n", __func__, w->name, event);
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		pr_debug("%s: sleeping 50ms before %s hph dac1 turn on\n",
				__func__, w->name);
...
static const struct snd_soc_dapm_widget sitar_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MIXER_E("DAC1", SITAR_A_RX_EAR_EN, 6, 0, dac1_switch,
		ARRAY_SIZE(dac1_switch), sitar_codec_enable_dac1,
	...
	SND_SOC_DAPM_MICBIAS_E("MIC BIAS1 External", SITAR_A_MICB_1_CTL, 7, 0,
		...
		SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_POST_PMD),
	/*add by youjin:to open amic3*/
	SND_SOC_DAPM_MICBIAS_E("MIC BIAS1 External2", SITAR_A_MICB_1_CTL, 7, 0,
		sitar_codec_enable_micbias, SND_SOC_DAPM_PRE_PMU |
		SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_POST_PMD),
...
static const struct snd_soc_dapm_route audio_map[] = {
	...
	/*add by youjin*/
	{"MIC BIAS1 External", NULL, "LDO_H"},
	{"MIC BIAS1 External2", NULL, "LDO_H"},
};
	...
	struct snd_soc_dai *dai)
{
	...
	/* added by maleijie 20121126 for ear pop. */
	usleep_range(100000, 100000);
	pr_debug("%s(): substream = %s  stream = %d\n" , __func__,
		substream->name, substream->stream);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		pr_debug("ch_act:%d ch_tot:%d\n",
			sitar_p->dai[j].ch_act, sitar_p->dai[j].ch_tot);
		if (sitar_p->dai[j].ch_act == sitar_p->dai[j].ch_tot) {
			ret = sitar_codec_enable_chmask(sitar_p, event, j);
			pr_debug("PMU: w->sname[%s]:j:%d,ch_num:%d,ch_tot:%d,rate:%d\n",
				w->sname, j, *(sitar_p->dai[j].ch_num),
				sitar_p->dai[j].ch_tot, sitar_p->dai[j].rate);
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		pr_debug("PMD:w->sname[%s]:j:%d,ch_num:%d,ch_tot:%d,\n",
			w->sname, j, *(sitar_p->dai[j].ch_num),
			sitar_p->dai[j].ch_tot);
		if (!sitar_p->dai[j].ch_act) {
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	pr_debug("en %s---- event:%d ,%s:ch_act:[%d]\n",
		w->sname, event, (event == 0x2) ? "PMU" : "PMD",
		sitar_p->dai[j].ch_act);
				   ...
				   bool override_bypass, bool noreldetection)
{
	...
	if (dce) {
		...
		snd_soc_write(codec, SITAR_A_CDC_MBHC_EN_CTL, 0x4);
		/* added by lifei 2013.1.23 */
		pr_debug("%s: sitar->mbhc_data.t_dce=%d\n",
			__func__, sitar->mbhc_data.t_dce);
				 ...
				 u16 bias_value)
{
	...
	pr_debug("[%s]dec_z:%d dec_mb%d, bias_value:%d mv:%d\n",
		__func__, z, mb, value, mv);
...
void sitar_mbhc_cal(struct snd_soc_codec *codec)
{
	...
	/* added by lifei 2013.1.23 for button detect */
	sitar->mbhc_data.t_dce = (DEFAULT_DCE_WAIT - 5000);
...
static void sitar_codec_decide_gpio_plug(struct snd_soc_codec *codec)
{
	...
	pr_debug("%s: DCE run 1, mic_mv[0] = %d, mb_v[0] = %d\n",
		__func__, mic_mv[0], mb_v[0]);
...
static void sitar_hs_gpio_handler(struct snd_soc_codec *codec)
{
	...
	if ((priv->current_plug == PLUG_TYPE_NONE) && insert) {
		pr_debug("%s: here insert:%d, priv->current_plug:%d\n",
			__func__, insert, priv->current_plug);
		/* add by youjin to solve the bug;2012-11-26 */
		sitar_codec_enable_adc_block(codec, 1);
...
static void mbhc_fw_read(struct work_struct *work)
{
	...
	if (ret != 0) {
		...
		/* fengchunsong use mbhc_cfg's calibration instead of sitar's */
