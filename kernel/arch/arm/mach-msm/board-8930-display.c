...
static int mipi_dsi_cdp_panel_power(int on)
{
	...
	if (!dsi_power_on) {
		...
		if (regulator_count_voltages(reg_l23) > 0) {
...
static struct mipi_dsi_panel_platform_data global_pdata = {
	#ifdef CONFIG_BOARD_VER_T0
	.enable_wled_bl_ctrl = 0x1,
	#else
	.enable_wled_bl_ctrl = 0x0,
...
static struct platform_device mipi_dsi_global_panel_device = {
	.name = "mipi_global",
	.id = 0,
	.dev = {
		.platform_data = &global_pdata,
	}
};
#endif
/*end*/
