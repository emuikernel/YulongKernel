...
void *pil_get(const char *name)
{
	...
	if (!pil->count) {
		ret = load_image(pil);
		pr_info("%s:%d: count is 0, load '%s' image return value: %d.\n",
				__func__, __LINE__, pil->desc->name, ret);
		if (ret) {
			retval = ERR_PTR(ret);
			goto err_load;
		}
	}
	pr_info("%s: '%s' depends_on = '%s', count: %d", __func__,
			pil->desc->name, pil->desc->depends_on == NULL ?
			"none" : pil->desc->depends_on, pil->count);
	pil->count++;
...
void pil_put(void *peripheral_handle)
{
	...
	pr_info("%s:%d '%s', count: %d", __func__,
			__LINE__, pil->desc->name, pil->count);

	if (!--pil->count) {
		pr_info("%s: count is 0, now it's time to pil_shutdown '%s'\n",
				__func__, pil->desc->name);
		pil_shutdown(pil);
	}
	mutex_unlock(&pil->lock);
