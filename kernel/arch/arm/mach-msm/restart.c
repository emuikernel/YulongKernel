...
static void set_dload_mode(int on)
{
	download_enable = on;
	if (restart_reason)
		__raw_writel(on ? 0x594c444c : 0, restart_reason);
	if (dload_mode_addr) {
...
{
	local_irq_disable(); /* 解决关机死机.fengchunsong.2012.03.10 */
...
void msm_restart(char mode, const char *cmd)
{
	...
	pm8xxx_smpl_control(0);

	if (download_enable == 0) {
		/* modified by shchj 20130301
		__raw_writel(0, MSM_IMEM_BASE + 0x658);
		*/
	if (cmd != NULL) {
		if (!strncmp(cmd, "bootloader", 10)) {
			__raw_writel(0x77665500, restart_reason);
			} else if (!strncmp(cmd, "recovery-over", 13)) {
				__raw_writel(0x594c5256, restart_reason);
...
static int __init msm_pmic_restart_init(void)
{
	...
	/* If download mode is not enabled, watch dog reset is normal reset.*/
	if (download_mode == 0)
		/*modified by shchj 20130301*/
		__raw_writel(0xdeadbeef, restart_reason);

	pr_info("MSM_IMEM_BASE:0x%x, restart_reason=0x%x\n",
			(unsigned int)MSM_IMEM_BASE,
			__raw_readl(MSM_IMEM_BASE + RESTART_REASON_ADDR));

	pp = (unsigned int *)
		(smem_get_entry(SMEM_POWER_ON_STATUS_INFO, &smem_size));
	poweron_reason = *(unsigned int *)
		(smem_get_entry(SMEM_POWER_ON_STATUS_INFO, &smem_size));
	pr_info("poweron_reason  0x%x=%x\n", (unsigned int)pp, *pp);

	pp = (unsigned int *)(smem_get_entry(SMEM_HW_RESET_DETECT, &smem_size));
	pr_info("hw reset  0x%x=%x\n", (unsigned int)pp, *pp);
