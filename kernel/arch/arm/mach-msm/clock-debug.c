...
/* add by chenfeng */
static char ClockOnOff[4] = {0};
static struct class *clockprintonoff_class;
...
/* add chenfeng */
void clock_print_yl(int force)
{
	char *value = "on";
	if (!force && strcmp(value, ClockOnOff))
		return;
	else
...
static ssize_t clock_onoff_show(struct class *class,
		struct class_attribute *attr, char *buf)
{
	pr_debug("%s,clock_onoff status=%s\n", __func__, buf);
	return sprintf(buf, "%s\n", ClockOnOff);
}

static ssize_t clock_onoff_store(struct class *cls,
...
static int clock_debug_add(struct clk *clock)
{
	...
	if (NULL == clockprintonoff_class) {
		clockprintonoff_class =
			class_create(THIS_MODULE, "clockprintonoff");
		if (IS_ERR(clockprintonoff_class))
			pr_err("%s: failed to create clockprintonoff_class\n",
				__func__);
		error = class_create_file(clockprintonoff_class,
			(const struct class_attribute *)&class_attr_clock_onoff.attr);
		if (error < 0)
			pr_err("failed to create class_attr_clock_onoff\n");
	}
